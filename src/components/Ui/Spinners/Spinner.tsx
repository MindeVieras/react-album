...
const Spinner = ({ classes, type, size }: any) => {
  let spinnerClass = classes.spinnerWrapper
  let spinner = <span />

  if (type === 'primary') {
    spinner = <RingLoader color={'#f6f6f5'} size={size} />
  } else if (type === 'list-item') {
    spinnerClass = `${classes.spinnerWrapper} ${classes.listSpinnerWrapper}`
    spinner = <ScaleLoader color={'#f6f6f5'} height={size} />
  } else if (type === 'thumbnail') {
    spinner = <ClipLoader color={'#f6f6f5'} size={size} />
  }
